function result = var_robustness_check(data, prior, gssettings, printout)

% This is the script for the unrestricted version of the BVAR model
% in the robustness check section
% model setup: full model, no zero restrictions, full interactions between
% both low and high frequency variables
...
Nm = 0; % now Nm contains nothing and Ny contains all variables 2+5
Ny = N - Nm;
P = prior.lags;
...
%=======================================================================
% Minnesota prior. Nothing changed in this part
if isfield(prior,'minnesota') && isfield(prior.minnesota,'tightness')
    ...
    end % above doesn't need change
    
    % prior for the coefficients
    % p(B|Sigma) = N( vecB0, Q0 )
    if ~isinf(prior.minnesota.tightness) % yes because tightness = 0.2
        ...
        elseif length(prior.minnesota.mvector)==Ny
            prior.minnesota.mvector = [zeros(Nm,1); prior.minnesota.mvector(:)]; % now the zeros are empty because Nm=0
        end
        prior.B(1:N,1:N) = diag(prior.minnesota.mvector(1:N));
        %prior.B(:,1:Nm) = []; % drop the equations for m, commented out
        prior.QinvB_reshaped = reshape(prior.Qinv*prior.B(:), K, Ny);
    end
    
    %========================================================================
    % Sims' dummy observations, omitted
...
if gssettings.ndraws
    ...
    ndraws = gssettings.ndraws; % number of saved draws
    nalldraws = gssettings.burnin + ndraws * gssettings.saveevery; % number of total iterations
    
    % allocate space for the posterior draws
    % empty structures to save draws
    ...
    % last preparations, set up a timer
    it_save = 0;
    ...
    for draw = 1:nalldraws
        ...
        % save current iteration if appropriate (if draw/saveevery = 0)
        if draw>gssettings.burnin && ~rem(draw,gssettings.saveevery)
            ...
            if gssettings.computemarglik % usually it is set to zero and doesn't run
                if nnans
    ...
    if gssettings.computemarglik % not in my case
