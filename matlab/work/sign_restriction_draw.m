function irfs_draws = sign_restriction_draw(res, nstep, dims_array, test_restr, b_normalize, max_attempts)

% This script is to recover structural shocks from reduced form VAR results.
...
C_saved = nan(N,N,ndraws);
Q_saved = nan(N,N,ndraws);
...
for i = 1:ndraws
    ...
    C = chol(sigmadraw);
    % choleski irf
    irfchol = impulsdtrf(reshape(betadraw',N,N,P),C,nstep);
    ...
    for a = 1:max_attempts
        ...
        % the above operation creates the Q with the left top part being Q*
