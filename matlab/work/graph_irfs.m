...
% This script is to construct IRFs to the two identified structural shocks
% it is modified based on Jarocinski and Karadi (2020)

function handles = graph_irfs(irfs_draws, varstoplot, shockstoplot, ...
...
if whichplots(1)
    ...
    for is = 1:length(shockstoplot)
        ...
        for iv = 1:length(varstoplot);
            ...
            hold on
            fill([tt' flipud(tt)'], [toplot(:,4)' flipud(toplot(:,5))'], [0.8 0.8 0.8], 'EdgeColor', 'none')
            fill([tt' flipud(tt)'], [toplot(:,2)' flipud(toplot(:,3))'], [0.6 0.6 0.6], 'EdgeColor', 'none')
